...
const spotifyAPI=new spotifyWrapper({ //used to make all API calls, if committing to github, remove the clientId and clientSecret. 
    clientId:"YOUR CLIENTID HERE",
    clientSecret:"YOUR CLIENTSECRET HERE",
    //redirectUri:"http://localhost:3001/getAccessToken/"
    redirectUri:"http://localhost:3000/profile"//name the endpoint whatever you like.
...
app.get("/login",(request,response)=>{ //frontend calls this endpoint. This endpoint should use the spotify api wrapper to call the authorise endpoint
...
app.get("/getAccessToken",(request,response)=>{ 
    ...
    spotifyAPI.authorizationCodeGrant(code).then( (data)=> {
         ...
         /* spotifyAPI.setAccessToken(data.body['access_token']);
          ...
          //IDEA: SEND THE expiresIn variable as a response to client, and call the setInterval function from there
...
//instead of calling this endpoint, maybe convert it into a function and then call it?
// or copy the setInterval from above, do it on the client side and call the refreshAcessToken endpoint from the clientSide?
...
app.get("/profile",(request,response)=>{ //this endpoint is called by the client side
    ...
    //API Call will fail if authentication has not been given, so need to handle that(either display error message or redirect to login endpoint)
})

//TODO:Frontend also has to display top 5 tracks and top 5 artists in separate cards


app.get("/checkCookies",(request,response)=>{
    console.log("Cookies:",request.cookies);
    response.json({message:"Just checking if the cookie parser works xD"});
})

app.get("/getFavouriteArtists",(request,response)=>{
    console.log("Cookies",request.cookies);
    var accessToken=request.cookies.accessToken;
    spotifyAPI.setAccessToken(accessToken);
...
app.get("/getFavouriteTracks", (request,response)=>{
...
//the code block below this comment follows the client credentials authorisation flow. This allows the app to access general Spotify endpoints(without personalised data of a user)

/*spotifyAPI.clientCredentialsGrant().then((data)=>{ //returns an access token for the app(user is not involved)
